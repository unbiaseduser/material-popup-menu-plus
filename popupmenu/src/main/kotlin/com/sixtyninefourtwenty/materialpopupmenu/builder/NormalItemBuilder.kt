...
sealed class NormalItemBuilder<T : NormalItemBuilder<T>> : AbstractItemBuilder<T>() {
    ...
    fun setLabelTypeface(labelTypeface: Typeface) = self().apply { data.labelTypeface = labelTypeface }
    fun setLabelAlignment(@GravityInt labelAlignment: Int) = self().apply { data.labelAlignment = labelAlignment }
