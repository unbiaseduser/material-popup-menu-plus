...
internal class MaterialRecyclerViewPopupWindow(
    ...
    private val popup = createAppCompatPopupWindow(context, customAnimation)
    ...
    init {
        ...
        this.dropDownHorizontalOffset = if (dropDownHorizontalOffset != DISABLED_INT) dropDownHorizontalOffset else a.getDimensionPixelOffset(
                R.styleable.MaterialRecyclerViewPopupWindow_android_dropDownHorizontalOffset, 0)
        this.dropDownVerticalOffset = if (dropDownVerticalOffset != DISABLED_INT) dropDownVerticalOffset else a.getDimensionPixelOffset(
    ...
    internal fun show() {
        ...
        if (popup.isShowing) {
            ...
            popup.update(view, dropDownHorizontalOffset, dropDownVerticalOffset, widthSpec, if (height < 0) -1 else height)
        } else {
            ...
            popup.isOutsideTouchable = true
            PopupWindowCompat.showAsDropDown(popup, view, dropDownHorizontalOffset, dropDownVerticalOffset, dropDownGravity)
        }
    ...
    private fun generateDefaultLayoutParams() = RecyclerView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT)

    private fun setPopupClipToScreenEnabled() =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            popup.setIsClippedToScreen(true)
        } else clipToWindowEnabledMethod?.let {
            try {
                it.invoke(popup, true)
            } catch (e: Exception) {
                Log.i(TAG, "Could not call setClipToScreenEnabled() on PopupWindow. Oh well.")
            }
        }

    private fun getMaxAvailableHeight(anchor: View, yOffset: Int, ignoreBottomDecorations: Boolean) =
